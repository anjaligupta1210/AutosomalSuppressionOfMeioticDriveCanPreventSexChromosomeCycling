#Simulation: Test invasion of autosomal suppressor where the initial population is at equilibrium for X-linked driver and Y-linked suppressor
#Code: Anjali Gupta
...
result_all <- data.frame(ssrm=numeric(),hsr=numeric(),ssr=numeric(),ha=numeric(),sa=numeric(),sy=numeric(),d=numeric(),
                         ...
                         DriveStable=character(),AutosomeStable=character(),YStable=character(),AsupInvade=character())


a <- seq(0,1,0.01)
b <- seq(0,0.5,0.01)

#This nested "for loop" loops over a range of possible parameters and tests for the invasion of an autosomal suppressor in a population that is at equilibrium for X-linked drivers and Y-linked suppressors

for(ssrm in 0){
  for(ssr in a){
    for(sa in c(0,0.3,0.5,1)){
      for(hsr in 0){
        for(ha in 0){
          for(sy in c(0.1,0.3,0.5,1)) {
            for(d in b) {
    
    p1=0.49 #XXAA ##a is autosomal suppressor ##A is standard autosome 
    p2=0 #XXAa
    p3=0 #XXaa
    p4=0.01 #XsrXAA ##Xsr is X-linked driver ##X is standard X 
    p5=0 #XsrXAa
    p6=0 #XsrXaa
    p7=0 #XsrXsrAA
    p8=0 #XsrXsrAa
    ...
    if((!is.na(t)==TRUE) &&
      ...
      if(var(result_1$XSRm[(t-500):t])<1e-5 && 
        ...
        #This loop tests for invasion of autosomal suppressor and introduces the autosomal suppressor into the population at equilibrium
        ...
        if((!is.na(t)==TRUE) &&
          ...
          #result_all dataframe contains information for populations and parameter spaces that we tested for invasion of autosomal suppressor
          ...
          result_all[length(result_all$ssrm)+1,]=c(ssrm,hsr,ssr,ha,sa,sy,d,
                                                   ...
                                                   AsupInvade=if(max(result_2$Asupm[3:length(result_2$Asupm)] > 0.005)) "Yes" else "No")
...
write.csv(result_all,"AsupInvasion_EqbmPop_Fig3_S6-8.csv",row.names=FALSE)

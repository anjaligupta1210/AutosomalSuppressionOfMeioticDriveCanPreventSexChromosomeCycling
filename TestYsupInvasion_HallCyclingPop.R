#Simulation: Test for invasion of Y-linked suppressor and if cycling occurs in stable cycling regions in populations initially at equilibrium for X-linked driver and autosomal suppressor
#Code: Anjali Gupta
#Last worked on: 15 September 2023
...
Results <- data.frame(ssrm=numeric(), ssr=numeric(), sy=numeric(), hsr=numeric(), d=numeric(), ha=numeric(), sa=numeric(),
                      YsupInvades=character(), AsupIsLost=character(), 
                      XsrYsupAsupEqbm=character(), 
                      XsrYsupCycling=character(), AsupXsrYsupCycling=character())
...
for(i in 1:nrow(Hall_Region4)) {
  ...
  for(ha in seq(0,1,0.5)){
    for(sa_2 in sy+sa_add){
      for(sa in c(0,sa_2)){
    ...
    result_all <- rbind(result_1, result_2)
    
    Data <- data.frame(ssrm=ssrm, ssr=ssr, sy=sy, hsr=hsr, d=d, ha=ha, sa=sa)
    
    Data$YsupInvades[1] <- if(max(result_2$Ysup[5:length(result_2$Ysup)] > 0.01)) "Yes" else "No"
    
    Data$AsupIsLost[1] <- if(max(result_2$Asupm[50000:length(result_2$Asupm)] < 0.001)) "Yes" else "No"
    
    Data$XsrYsupAsupEqbm[1] <- if(var(result_2$Ysup[(length(result_2$Ysup)-5000):length(result_2$Ysup)])<1e-5 &
                                  var(result_2$Asupm[(length(result_2$Asupm)-5000):length(result_2$Asupm)])<1e-5 &
                                  var(result_2$Asupf[(length(result_2$Asupf)-5000):length(result_2$Asupf)])<1e-5 &
                                  var(result_2$XSRm[(length(result_2$XSRm)-5000):length(result_2$XSRm)])<1e-5 &
                                  var(result_2$XSRf[(length(result_2$XSRf)-5000):length(result_2$XSRf)])<1e-5) "Yes" else "No"
    
    Data$XsrYsupCycling[1] <- if(Data$XsrYsupAsupEqbm[1]=="No" &
                                 Data$YsupInvades[1]=="Yes") "Stable Cycling" else
                                   if(Data$XsrYsupAsupEqbm[1]=="Yes" &
                                      Data$YsupInvades[1]=="Yes" &
                                      result_2$Ysup[length(result_2$Ysup)]<0.998 &
                                      result_2$Ysup[length(result_2$Ysup)]>0.002 &
                                      result_2$XSRm[length(result_2$XSRm)]<0.998 &
                                      result_2$XSRm[length(result_2$XSRm)]>0.002 &
                                      result_2$XSRf[length(result_2$XSRf)]<0.998 &
                                      result_2$XSRf[length(result_2$XSRf)]>0.002) "Dampened Cycling" else 
                                        if(Data$XsrYsupAsupEqbm[1]=="Yes" &
                                           Data$YsupInvades[1]=="Yes" &
                                           var(result_2$Ysup[(length(result_2$Ysup)-30000):length(result_2$Ysup)])>1 &
                                           var(result_2$XSRm[(length(result_2$XSRm)-30000):length(result_2$XSRm)])>1 &
                                           var(result_2$XSRf[(length(result_2$XSRf)-30000):length(result_2$XSRf)])>1) "Irregular & Extended Cycling" else "No"
    
    Data$AsupXsrYsupCycling[1] <- if(Data$XsrYsupAsupEqbm[1]=="No" &
                                     Data$YsupInvades[1]=="Yes" &
                                     Data$AsupIsLost[1]=="No") "Stable Cycling" else
                                       if(Data$XsrYsupAsupEqbm[1]=="Yes" &
                                          Data$YsupInvades[1]=="Yes" & 
                                          Data$AsupIsLost[1]=="No" &
                                          result_2$Ysup[length(result_2$Ysup)]<0.998 &
                                          result_2$Ysup[length(result_2$Ysup)]>0.002 &
                                          result_2$XSRm[length(result_2$XSRm)]<0.998 &
                                          result_2$XSRm[length(result_2$XSRm)]>0.002 &
                                          result_2$XSRf[length(result_2$XSRf)]<0.998 &
                                          result_2$XSRf[length(result_2$XSRf)]>0.002 &
                                          result_2$Asupm[length(result_2$Asupm)]<0.998 &
                                          result_2$Asupm[length(result_2$Asupm)]>0.002 &
                                          result_2$Asupf[length(result_2$Asupf)]<0.998 &
                                          result_2$Asupf[length(result_2$Asupf)]>0.002) "Dampened Cycling" else 
                                            if(Data$XsrYsupAsupEqbm[1]=="Yes" &
                                               Data$YsupInvades[1]=="Yes" &
                                               Data$AsupIsLost[1]=="No" &
                                               var(result_2$Ysup[(length(result_2$Ysup)-30000):length(result_2$Ysup)])>1 &
                                               var(result_2$XSRm[(length(result_2$XSRm)-30000):length(result_2$XSRm)])>1 &
                                               var(result_2$XSRf[(length(result_2$XSRf)-30000):length(result_2$XSRf)])>1 &
                                               var(result_2$Asupm[(length(result_2$Asupm)-30000):length(result_2$Asupm)])>1 &
                                               var(result_2$Asupf[(length(result_2$Asupf)-30000):length(result_2$Asupf)])>1) "Irregular & Extended Cycling" else "No"
...
write.csv(Results, "TestYsupInvasion_HallRegionIV_CyclingPop.csv", row.names = FALSE)